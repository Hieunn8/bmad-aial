...
I want to deliver **Oracle Execution with Identity Passthrough + VPD Enforcement** with measurable acceptance checks,
so that Epic 2A can progress safely with verifiable outcomes.

## Acceptance Criteria

1. Hành vi/đầu ra của story **2A.4 - Oracle Execution with Identity Passthrough + VPD Enforcement** bám đúng phạm vi mô tả trong `_bmad-output/planning-artifacts/epics.md`.
2. Tất cả yêu cầu liên quan được trace được về Epic 2A (ví dụ: Các yêu cầu chức năng/NFR liên quan trong epics.md).
...
- Epic context: **Epic 2A — Minimal Viable Query**.
...
- Primary artifact file: `_bmad-output/implementation-artifacts/2a-4-oracle-execution-with-identity-passthrough-plus-vpd-enforcement.md`.
...
- `_bmad-output/planning-artifacts/epics.md` — Story 2A.4

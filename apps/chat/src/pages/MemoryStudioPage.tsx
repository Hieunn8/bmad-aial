...
export function MemoryStudioPage(): React.JSX.Element {
  ...
  async function handleSaveTemplate(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      setFlash(`Da luu mau ${templateForm.name}`);
      await loadMemorySurfaces();
    } catch (templateError) {
      setError(templateError instanceof Error ? templateError.message : 'Luu mau that bai');
    }
  }
  ...
  async function handleReuse(entryId: string): Promise<void> {
    ...
    try {
      ...
      setFlash('Da nap lai intent vao khung soan thao');
    } catch (reuseError) {
      setError(reuseError instanceof Error ? reuseError.message : 'Khong dung lai duoc truy van');
    }
  }

  useEffect(() => {
    void Promise.all([loadMemorySurfaces(), loadHistory(), probeContext(contextQuery)]).catch((loadError: unknown) => {
      setError(loadError instanceof Error ? loadError.message : 'Khong tai duoc Memory Studio');
    });
  ...
  return (
    <section style={pageShell}>
      ...
      <p style={{ margin: '0.45rem 0 1.2rem', color: 'var(--color-neutral-600)' }}>
        Kiem tra selective recall, saved templates, suggestions va lich su hoi thoai.
      </p>
      {(flash || error) && (
      ...
      <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: '1.2fr 0.8fr' }}>
        <section style={{ ...cardStyle, padding: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
            <h2 style={{ margin: 0, fontSize: '1.2rem' }}>Context probe</h2>
          ...
          <p style={{ color: 'var(--color-neutral-600)' }}>Token delta: {memoryProbe?.token_budget_increase_percent ?? 0}%</p>
        ...
        <aside style={{ display: 'grid', gap: '1rem' }}>
          <div style={{ ...cardStyle, padding: '1.25rem' }}>
            <h2 style={{ margin: 0, fontSize: '1.1rem' }}>Suggestions</h2>
            {suggestions.map((suggestion) => (
              <div key={`${suggestion.type}-${suggestion.label}`} style={{ marginTop: '0.75rem', borderRadius: '0.9rem', background: 'rgba(15,118,110,0.08)', padding: '0.8rem' }}>
                ...
                <div style={{ color: 'var(--color-neutral-500)' }}>{suggestion.type === 'kpi' ? `${suggestion.uses ?? 0} lan dung` : 'Saved template'}</div>
              </div>
            ))}
          </div>
          <div style={{ ...cardStyle, padding: '1.25rem' }}>
            <h2 style={{ margin: 0, fontSize: '1.1rem' }}>Memory self-audit</h2>
            <p style={{ color: memoryAudit.length ? '#991b1b' : '#166534' }}>
              {memoryAudit.length === 0 ? 'Khong co raw-value violation cho user hien tai.' : `${memoryAudit.length} violation`}
            </p>
          </div>
        </aside>
      </div>
      ...
      <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: '1.2fr 0.8fr', marginTop: '1rem' }}>
        <form onSubmit={(event) => void handleSaveTemplate(event)} style={{ ...cardStyle, padding: '1.25rem', display: 'grid', gap: '0.8rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.2rem' }}>Save template</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.8rem' }}>
            <input value={templateForm.name} onChange={(event) => setTemplateForm((current) => ({ ...current, name: event.target.value }))} style={inputStyle} placeholder="Template name" />
            <input value={templateForm.output_format} onChange={(event) => setTemplateForm((current) => ({ ...current, output_format: event.target.value }))} style={inputStyle} placeholder="Output format" />
            <input value={templateForm.query_intent} onChange={(event) => setTemplateForm((current) => ({ ...current, query_intent: event.target.value }))} style={inputStyle} placeholder="Query intent" />
            <input value={templateForm.time_range} onChange={(event) => setTemplateForm((current) => ({ ...current, time_range: event.target.value }))} style={inputStyle} placeholder="Time range" />
          </div>
          <textarea value={templateForm.filters} onChange={(event) => setTemplateForm((current) => ({ ...current, filters: event.target.value }))} style={{ ...inputStyle, minHeight: '4.5rem' }} placeholder="Filters" />
          <button type="submit" style={buttonStyle}>Save Template</button>
        </form>
        <aside style={{ ...cardStyle, padding: '1.25rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.1rem' }}>Saved templates</h2>
      ...
      <section style={{ ...cardStyle, padding: '1.25rem', marginTop: '1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.2rem' }}>Conversation History</h2>
          <button type="button" style={ghostButtonStyle} onClick={() => void loadHistory()}>Search</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '0.8rem', marginTop: '1rem' }}>
          <input value={historyKeyword} onChange={(event) => setHistoryKeyword(event.target.value)} style={inputStyle} placeholder="Keyword" />
          <input value={historyTopic} onChange={(event) => setHistoryTopic(event.target.value)} style={inputStyle} placeholder="Topic" />
          <button type="button" style={buttonStyle} onClick={() => void loadHistory()}>Run search</button>
        </div>
        {historyEntries.map((entry) => (
          <article key={entry.entry_id} style={{ marginTop: '0.85rem', borderRadius: '1rem', border: '1px solid rgba(117, 94, 60, 0.14)', padding: '0.95rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
              ...
              <button type="button" style={ghostButtonStyle} onClick={() => void handleReuse(entry.entry_id)}>Dung lai</button>
            </div>
        ...
        <div style={{ marginTop: '1rem', borderRadius: '1rem', background: 'rgba(26,31,44,0.95)', color: '#d7f9f1', padding: '1rem' }}>
          <strong>Preloaded draft</strong>
          <div style={{ marginTop: '0.4rem' }}>{draftPrompt || 'Chua co intent nao duoc nap lai.'}</div>
        </div>
      </section>
    </section>
  );

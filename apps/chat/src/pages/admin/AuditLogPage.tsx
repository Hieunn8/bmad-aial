...
export function AuditLogPage(): React.JSX.Element {
  ...
  useEffect(() => {
    void load().catch((loadError: unknown) => setError(loadError instanceof Error ? loadError.message : 'Khong tai duoc audit log'));
  ...
  return (
    <AdminPageShell title="Audit Log" actions={<button type="button" style={ghostButtonStyle} onClick={() => void handleExportCsv()}>Xuat CSV</button>}>
      ...
      <div style={{ ...adminCard, display: 'grid', gridTemplateColumns: 'repeat(6, minmax(0, 1fr))', gap: '0.8rem', marginBottom: '1rem' }}>
        ...
        <input value={filters.user_id} onChange={(event) => { setPage(1); setFilters((current) => ({ ...current, user_id: event.target.value })); }} style={inputStyle} placeholder="User ID" />
        <input value={filters.action} onChange={(event) => { setPage(1); setFilters((current) => ({ ...current, action: event.target.value })); }} style={inputStyle} placeholder="Action" />
        ...
        <button type="button" style={buttonStyle} onClick={() => void load()}>Tim kiem</button>
      </div>
      <div style={adminCard}>
        <table style={tableStyle}>
          <thead>
            <tr><th style={cellStyle}>Thoi gian</th><th style={cellStyle}>User</th><th style={cellStyle}>Action</th><th style={cellStyle}>Data Source</th><th style={cellStyle}>Rows</th><th style={cellStyle}>Latency</th><th style={cellStyle}>Policy</th><th style={cellStyle}>Chi tiet</th></tr>
          </thead>
          <tbody>
            {records.map((record) => (
              <tr key={`${record.request_id}-${record.timestamp}`}>
                ...
                <td style={cellStyle}><button type="button" style={ghostButtonStyle} onClick={() => { setSelected(record); setLifecycle([]); }}>Chi tiet</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        <div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button type="button" style={ghostButtonStyle} disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>Truoc</button>
          <span>Trang {page} / {totalPages}</span>
          <button type="button" style={ghostButtonStyle} disabled={page >= totalPages} onClick={() => setPage((current) => current + 1)}>Tiep theo</button>
        </div>
      </div>
      {selected && (
        <aside style={{ position: 'fixed', top: 'var(--header-height)', right: 0, bottom: 0, width: '30rem', overflowY: 'auto', background: 'white', borderLeft: '1px solid rgba(117,94,60,0.16)', padding: '1.2rem', zIndex: 20, boxShadow: '-18px 0 40px rgba(99,74,45,0.12)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
            <h2 style={{ margin: 0, fontSize: '1.2rem' }}>Chi tiet audit</h2>
            <button type="button" style={ghostButtonStyle} onClick={() => setSelected(null)}>Dong</button>
          </div>
          <dl style={{ display: 'grid', gap: '0.7rem', marginTop: '1rem' }}>
            <dt>Request ID</dt><dd>{selected.request_id} <button type="button" onClick={() => void navigator.clipboard.writeText(selected.request_id)}>Copy</button></dd>
            <dt>Thoi gian</dt><dd>{new Date(selected.timestamp).toLocaleString('vi-VN')}</dd>
            <dt>User / Department</dt><dd>{selected.user_id} / {selected.department_id}</dd>
            <dt>Action</dt><dd>{selected.intent_type}</dd>
            <dt>Data sources</dt><dd>{selected.data_sources.join(', ')}</dd>
            <dt>SQL hash</dt><dd>{selected.sql_hash ?? 'none'}</dd>
            <dt>Rows / Latency</dt><dd>{selected.rows_returned} / {selected.latency_ms}ms</dd>
            <dt>Policy</dt><dd style={{ color: selected.policy_decision === 'ALLOW' ? '#166534' : '#991b1b', fontWeight: 700 }}>{selected.policy_decision}</dd>
            {selected.denial_reason && <><dt>Denial reason</dt><dd>{selected.denial_reason}</dd></>}
          </dl>
